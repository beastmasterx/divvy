...
class TestTransactionRepository:
    ...
    async def test_get_all_transactions_empty(self, db_session: AsyncSession):
    ...
    async def test_get_all_transactions_multiple(self, db_session: AsyncSession):
    ...
    async def test_get_transaction_by_id_exists(self, db_session: AsyncSession):
        ...
        retrieved = await repo.get_transaction_by_id(transaction_id)
    ...
    async def test_get_transaction_by_id_not_exists(self, db_session: AsyncSession):
        ...
        repo = TransactionRepository(db_session)
        result = await repo.get_transaction_by_id(99999)
    ...
    async def test_get_transactions_by_period_id(self, db_session: AsyncSession):
        ...
        period_1_transactions = await repo.get_transactions_by_period_id(1)
    ...
    async def test_get_shared_transactions_by_user_id(self, db_session: AsyncSession):
        ...
        transactions = await repo.get_shared_transactions_by_user_id(1)
        assert isinstance(transactions, list)
    ...
    async def test_create_transaction(self, db_session: AsyncSession):
        ...
        retrieved = await repo.get_transaction_by_id(created.id)
        assert retrieved is not None
        assert retrieved.amount == 15000

    async def test_update_transaction(self, db_session: AsyncSession):
        ...
        updated = await repo.update_transaction(transaction)

        assert updated.description == "Updated Description"
        assert updated.amount == 20000

        # Verify the update persisted
        retrieved = await repo.get_transaction_by_id(transaction.id)
        assert retrieved is not None
    ...
    async def test_delete_transaction_not_exists(self, db_session: AsyncSession):
        """Test deleting a transaction that doesn't exist (should not raise error)."""
        repo = TransactionRepository(db_session)
        # Should not raise an exception
        await repo.delete_transaction(99999)
