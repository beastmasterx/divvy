...
class TestUserIdentityRepository:
    ...
    def test_get_identity_by_id_exists(self, db_session: Session):
        ...
        retrieved = repo.get_identity_by_id(identity_id)
    ...
    def test_get_identity_by_id_not_exists(self, db_session: Session):
        """Test retrieving a user identity by ID when it doesn't exist."""
        repo = UserIdentityRepository(db_session)
        result = repo.get_identity_by_id(99999)
        assert result is None

    def test_get_identity_by_provider_and_external_id_exists(self, db_session: Session):
        ...
        retrieved = repo.get_identity_by_provider_and_external_id("google", "google_456")
        assert retrieved is not None
    ...
    def test_get_identity_by_provider_and_external_id_not_exists(self, db_session: Session):
        """Test retrieving a user identity by provider and external ID when it doesn't exist."""
        repo = UserIdentityRepository(db_session)
        result = repo.get_identity_by_provider_and_external_id("microsoft", "nonexistent")
        assert result is None

    def test_get_identities_by_user_id_empty(self, db_session: Session):
        ...
        identities = repo.get_identities_by_user_id(user.id)
        assert isinstance(identities, list)
        assert len(identities) == 0

    def test_get_identities_by_user_id_multiple(self, db_session: Session):
    ...
    def test_get_identities_by_provider_empty(self, db_session: Session):
        """Test retrieving identities for a provider when none exist."""
        repo = UserIdentityRepository(db_session)
        identities = repo.get_identities_by_provider("microsoft")
        assert isinstance(identities, list)
        assert len(identities) == 0

    def test_get_identities_by_provider_multiple(self, db_session: Session):
    ...
    def test_create_identity(self, db_session: Session):
        ...
        created = repo.create_identity(identity)
        ...
        retrieved = repo.get_identity_by_id(created.id)
        assert retrieved is not None
        assert retrieved.external_id == "fb_789"

    def test_update_identity(self, db_session: Session):
    ...
    def test_delete_identity_exists(self, db_session: Session):
        ...
        # Delete it
        repo.delete_identity(identity_id)

        # Verify it's gone
        retrieved = repo.get_identity_by_id(identity_id)
        assert retrieved is None

    def test_delete_identity_not_exists(self, db_session: Session):
        """Test deleting a user identity that doesn't exist (should not raise error)."""
        repo = UserIdentityRepository(db_session)
        # Should not raise an exception
        repo.delete_identity(99999)
