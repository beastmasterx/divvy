...
class TestAccountLinkRequestRepository:
    ...
    async def test_get_request_by_id_exists(self, db_session: AsyncSession):
        ...
        retrieved = await repo.get_request_by_id(request_id)
    ...
    async def test_get_request_by_id_not_exists(self, db_session: AsyncSession):
        """Test retrieving an account link request by ID when it doesn't exist."""
        repo = AccountLinkRequestRepository(db_session)
        result = await repo.get_request_by_id(99999)
        assert result is None

    async def test_get_request_by_token_exists(self, db_session: AsyncSession):
        ...
        retrieved = await repo.get_request_by_token("unique_token_789")
        assert retrieved is not None
        assert retrieved.request_token == "unique_token_789"
        assert retrieved.user_identity_id == identity.id

    async def test_get_request_by_token_not_exists(self, db_session: AsyncSession):
        """Test retrieving an account link request by token when it doesn't exist."""
        repo = AccountLinkRequestRepository(db_session)
        result = await repo.get_request_by_token("nonexistent_token")
        assert result is None

    async def test_get_requests_by_user_identity_id_empty(self, db_session: AsyncSession):
        ...
        requests = await repo.get_requests_by_user_identity_id(identity.id)
        assert isinstance(requests, list)
        assert len(requests) == 0

    async def test_get_requests_by_user_identity_id_multiple(self, db_session: AsyncSession):
    ...
    async def test_get_pending_requests_by_user_identity_id(self, db_session: AsyncSession):
        ...
        pending_requests = await repo.get_pending_requests_by_user_identity_id(identity.id)
    ...
    async def test_get_expired_requests(self, db_session: AsyncSession):
        ...
        expired_requests = await repo.get_expired_requests()
    ...
    async def test_create_request(self, db_session: AsyncSession):
        ...
        retrieved = await repo.get_request_by_id(created.id)
        assert retrieved is not None
        assert retrieved.request_token == "new_token_123"

    async def test_update_request(self, db_session: AsyncSession):
    ...
    async def test_delete_request_not_exists(self, db_session: AsyncSession):
        """Test deleting an account link request that doesn't exist (should not raise error)."""
        repo = AccountLinkRequestRepository(db_session)
        # Should not raise an exception
        await repo.delete_request(99999)
