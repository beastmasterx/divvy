...
class TestAuthService:
    ...
    def test_register_new_user(self, auth_service: AuthService, db_session: Session) -> None:
    ...
    def test_register_duplicate_email_raises_error(self, auth_service: AuthService, db_session: Session) -> None:
    ...
    def test_authenticate_valid_credentials(self, auth_service: AuthService, db_session: Session) -> None:
    ...
    def test_authenticate_invalid_password(self, auth_service: AuthService, db_session: Session) -> None:
    ...
    def test_authenticate_inactive_user(self, auth_service: AuthService, db_session: Session) -> None:
    ...
    def test_change_password_valid(self, auth_service: AuthService, db_session: Session) -> None:
    ...
    def test_change_password_invalid_old_password(self, auth_service: AuthService, db_session: Session) -> None:
    ...
    def test_change_password_user_not_found(self, auth_service: AuthService, db_session: Session) -> None:
    ...
    def test_reset_password(self, auth_service: AuthService, db_session: Session) -> None:
        ...
        updated_user = auth_service.reset_password(user_id=user_id, request=request)
    ...
    def test_reset_password_user_not_found(self, auth_service: AuthService, db_session: Session) -> None:
        """Test resetting password for non-existent user raises NotFoundError."""
        request = PasswordResetRequest(new_password="newpassword")
        with pytest.raises(NotFoundError):
            auth_service.reset_password(user_id=99999, request=request)

    def test_verify_token_valid(self, auth_service: AuthService, db_session: Session) -> None:
        ...
        # Create a token
        token = auth_service.authenticate(email=user.email, password=password).access_token
    ...
    def test_verify_token_expired(self, auth_service: AuthService, db_session: Session) -> None:
    ...
    def test_generate_refresh_token(self, auth_service: AuthService, db_session: Session) -> None:
        ...
        refresh_token = auth_service.authenticate(email=user.email, password=password).refresh_token

        assert refresh_token is not None
        assert isinstance(refresh_token, str)
        assert len(refresh_token) > 0

    def test_revoke_refresh_token(self, auth_service: AuthService, db_session: Session) -> None:
        ...
        # Revoke it
        revoked_token = auth_service.revoke_refresh_token(refresh_token)
    ...
    def test_rotate_refresh_token(self, auth_service: AuthService, db_session: Session) -> None:
        ...
        old_token: str = auth_service.authenticate(email=user.email, password=password).refresh_token

        # Rotate it
        token_response = auth_service.rotate_refresh_token(old_token)
        ...
        with pytest.raises(UnauthorizedError):
            auth_service.rotate_refresh_token(old_token)

    def test_rotate_refresh_token_invalid_token(self, auth_service: AuthService) -> None:
        """Test rotating an invalid refresh token raises UnauthorizedError."""
        with pytest.raises(UnauthorizedError):
            auth_service.rotate_refresh_token("invalid_token")
