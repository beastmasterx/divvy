...
class TestSettlementService:
    ...
    @pytest.fixture
    def settlement_service(self, db_session: Session):
    ...
    def test_get_all_balances_empty_period(self, settlement_service: SettlementService, db_session: Session):
        ...
        balances = settlement_service.get_all_balances(period.id)

        assert isinstance(balances, dict)
        assert len(balances) == 0

    def test_get_all_balances_with_deposits(self, settlement_service: SettlementService, db_session: Session):
        ...
        db_session.commit()
        group = create_test_group(name="Test Group", owner_id=user1.id)
        ...
        transaction_service.create_transaction(deposit1)
        transaction_service.create_transaction(deposit2)
    ...
    def test_get_all_balances_with_expenses(self, settlement_service: SettlementService, db_session: Session):
        ...
        transaction_service.create_transaction(expense)
    ...
    def test_get_settlement_plan_period_not_exists(self, settlement_service: SettlementService, db_session: Session):
        """Test getting settlement plan for non-existent period raises NotFoundError."""
        with pytest.raises(NotFoundError):
            settlement_service.get_settlement_plan(99999)

    def test_get_settlement_plan_period_already_closed(
        self, settlement_service: SettlementService, db_session: Session
    ):
        ...
        with pytest.raises(BusinessRuleError):
            settlement_service.get_settlement_plan(period.id)

    def test_get_settlement_plan_no_settlement_category(
        self, settlement_service: SettlementService, db_session: Session
    ):
    ...
    def test_get_settlement_plan_with_balances(self, settlement_service: SettlementService, db_session: Session):
        ...
        plan = settlement_service.get_settlement_plan(period.id)
    ...
    def test_apply_settlement_plan(self, settlement_service: SettlementService, db_session: Session):
        ...
        # Apply settlement
        settlement_service.apply_settlement_plan(period.id)

        # Verify period is settled
        period_service = PeriodService(db_session)
        settled_period = period_service.get_period_by_id(period.id)
        assert settled_period is not None
        assert settled_period.is_closed is True

        # Verify settlement transactions were created
        settlement_transactions = transaction_service.get_transactions_by_period_id(period.id)
