...
class TestAuthenticationService:
    ...
    @pytest.fixture
    async def authentication_ervice(self, db_session: AsyncSession) -> AuthenticationService:
    ...
    async def test_register_new_user(
        self, authentication_ervice: AuthenticationService, db_session: AsyncSession
    ) -> None:
        """Test registering a new user."""
        token_response = await authentication_ervice.register(
    ...
    async def test_register_duplicate_email_raises_error(
        ...
        with pytest.raises(ConflictError):
            await authentication_ervice.register(
    ...
    async def test_authenticate_valid_credentials(
        ...
        token_response = await authentication_ervice.authenticate(
    ...
    async def test_authenticate_invalid_email(self, authentication_ervice: AuthenticationService) -> None:
        """Test authenticating with invalid email raises UnauthorizedError."""
        with pytest.raises(UnauthorizedError):
            await authentication_ervice.authenticate(
    ...
    async def test_change_password_valid(
        ...
        updated_user = await authentication_ervice.change_password(
            user_id=user_id,
    ...
    async def test_change_password_invalid_old_password(
        ...
        with pytest.raises(UnauthorizedError):
            await authentication_ervice.change_password(
                user_id=user.id,
    ...
    async def test_reset_password(self, authentication_ervice: AuthenticationService, db_session: AsyncSession) -> None:
        ...
        updated_user = await authentication_ervice.reset_password(user_id=user_id, request=request)
    ...
    async def test_reset_password_user_not_found(
        ...
        with pytest.raises(NotFoundError):
            await authentication_ervice.reset_password(user_id=99999, request=request)
    ...
    async def test_verify_token_valid(
        ...
        # Create a token
        token = (await authentication_ervice.authenticate(email=user.email, password=password)).access_token

        # Verify it
        payload = await authentication_ervice.verify_token(token)
    ...
    async def test_verify_token_invalid(self, authentication_ervice: AuthenticationService) -> None:
        """Test verifying an invalid JWT token raises UnauthorizedError."""
        with pytest.raises(UnauthorizedError):
            await authentication_ervice.verify_token("invalid.token.here")
    ...
    async def test_verify_token_expired(
        ...
        with pytest.raises(UnauthorizedError):
            await authentication_ervice.verify_token(token)
    ...
    async def test_generate_refresh_token(
        ...
        refresh_token = (await authentication_ervice.authenticate(email=user.email, password=password)).refresh_token
    ...
    async def test_revoke_refresh_token(
        ...
        # Revoke it
        revoked_token = await authentication_ervice.revoke_refresh_token(refresh_token)

        assert revoked_token is not None
        assert revoked_token.is_revoked is True
        with pytest.raises(UnauthorizedError):
            await authentication_ervice.rotate_refresh_token(refresh_token)
    ...
    async def test_rotate_refresh_token(
        ...
        old_token: str = (await authentication_ervice.authenticate(email=user.email, password=password)).refresh_token

        # Rotate it
        token_response = await authentication_ervice.rotate_refresh_token(old_token)
        ...
        with pytest.raises(UnauthorizedError):
            await authentication_ervice.rotate_refresh_token(old_token)

    async def test_rotate_refresh_token_invalid_token(self, authentication_ervice: AuthenticationService) -> None:
        """Test rotating an invalid refresh token raises UnauthorizedError."""
        with pytest.raises(UnauthorizedError):
            await authentication_ervice.rotate_refresh_token("invalid_token")
